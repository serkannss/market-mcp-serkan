...
`;
   ...
   * Fiyat analizi yap
   */
  async analyzePrices(priceComparison: PriceComparison): Promise<string> {
    try {
      const prompt = `
Ürün fiyat analizi:
...
`;
   ...
   */
  async compareMarkets(products: Product[]): Promise<string> {
    try {
      const marketData: { [key: string]: { totalPrice: number; productCount: number; products: string[] } } = {};
...
${Object.entries(marketData).map(([market, data]) => 
  `${market}: ${data.productCount} ürün, toplam ${data.totalPrice.toFixed(2)} TL, ortalama ${(data.totalPrice / data.productCount).toFixed(2)} TL`
).join('\n')}
...
`;
   ...
   */
  private generateFakeMarketData(keywords: string[]): string {
    let allFakeData = '';
  ...
  private generateFakeMarketDataForKeyword(keyword: string): string {
    ...
    const prices: { price: number; market: string; district: string; city: string; productName: string; unit: string }[] = [];
  ...
  private generateProductName(keyword: string): string {
  ...
  private generateResponseWithFakeData(message: string, fakeData: string): string {
    const responses = [
