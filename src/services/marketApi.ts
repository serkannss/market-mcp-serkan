...
export class MarketApiService {
  ...
  constructor(baseUrl: string = 'https://api.marketfiyati.org.tr/api/v2') {
   ...
   * ID ile ürün arama
   */
  async searchByIdentity(request: SearchByIdentityRequest): Promise<SearchResponse> {
    try {
   ...
   */
  async comparePrices(productId: string, location: MarketLocation): Promise<PriceComparison> {
    try {
   ...
   */
  async findNearbyMarkets(location: MarketLocation, distance: number = 5): Promise<Product[]> {
    try {
   ...
   */
  async findCheapestProducts(keywords: string, location: MarketLocation): Promise<Product[]> {
    try {
      const searchResult = await this.searchProducts({
        keywords,
        latitude: location.latitude,
   ...
   */
  async filterByMarket(keywords: string, marketName: string, location: MarketLocation): Promise<Product[]> {
    try {
      const searchResult = await this.searchProducts({
        keywords,
        latitude: location.latitude,
      ...
      const filteredProducts = searchResult.content.filter(product =>
        product.productDepotInfoList.some(depot => 
          depot.marketAdi.toLowerCase().includes(marketName.toLowerCase())
        )
      );
