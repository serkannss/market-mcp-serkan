...
app.post('/api/compare-prices', async (req, res) => {
  try {
    ...
    const location: MarketLocation = { latitude, longitude };
...
app.post('/api/ai/recommendations', async (req, res) => {
  try {
    ...
    // AI önerisi al
    const recommendation = await geminiService.getProductRecommendations(
      userQuery,
      products.content,
      location
    );
...
app.post('/api/ai/shopping-list', async (req, res) => {
  try {
    ...
    const shoppingList = await geminiService.generateShoppingList(
      items,
      budget,
      preferences
    );
